...
5. `left_pointer = max(left_pointer, old_index + 1)` — This is the part that tricked me. Since old_index could be very small if the new element has already appeared before. So it is important to take the maximum of current left pointer and old_index + 1.
