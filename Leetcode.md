...
- **Integers Rescaling — Reverse Integer:**  
...
- **Sorting**  
  Why is time complexity nlog(n)? Sorting functions work by dividing by two, look at each element, hence n, and then dividing by two again, hence log(n), hence nlog(n)
...
- **Possible Bipartition**
  ...
  4. when creating the heapmap, be aware that if it is one way or two way?
...
- **Big-O combination**
  ...
  1. Sum(n): O(n), O(n) — Improvement: running sum, O(1), O(n)
  2. What if super big size and memory is an issue? Need external storage or approximate techniques
  3. deque(): O(1), O(n) — Improvement: bucket array solution,

  **Time:** look for loops, nested part, do running total, use heap for sorting  
  **Space:** store last value instead of total + In-place modification (trade off)

  **input huge:** Bucketing Technique, sliding window, Bit Manipulation(space)

  **doesn't fit memory:** External sorting, Chunk processing, MapReduce, Database aggregation
